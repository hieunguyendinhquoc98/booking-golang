...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	...
	err := m.DB.QueryRowContext(ctx, stmt,
		res.FirstName, res.LastName, res.Email, res.Phone, res.StartDate, res.EndDate, res.RoomID, time.Now(), time.Now()).Scan(&newID)
	if err != nil {
		return 0, err
	}
	return newID, nil
...
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	...
	query := `select id, first_name, last_name, email, password, access_level, updated_at, created_at from rooms where id = $1`
...
func (m *postgresDBRepo) AllReservations() ([]models.Reservation, error) {
	...
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.created_at, r.updated_at, r.processed, rm.id,
		rm.room_name from reservations r left join rooms rm on (r.room_id = rm.id) order by r.start_date asc
    `
...
func (m *postgresDBRepo) AllNewReservations() ([]models.Reservation, error) {
	...
	var reservations []models.Reservation
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.created_at, r.updated_at, rm.id,
		rm.room_name from reservations r left join rooms rm on (r.room_id = rm.id) where r.processed = 0 order by r.start_date asc
    `
	rows, err := m.DB.QueryContext(ctx, query)
	if err != nil {
		return reservations, err
...
func (m *postgresDBRepo) GetReservationByID(id int) (models.Reservation, error) {
	...
	query := `
		select r.id, r.first_name, r.last_name, r.email, r.phone, r.start_date, r.end_date, r.room_id, r.created_at, r.updated_at, r.processed, rm.id,
		rm.room_name from reservations r left join rooms rm on (r.room_id = rm.id) where r.id = $1
    `
	row := m.DB.QueryRowContext(ctx, query, id)
	err := row.Scan(
		&reservation.ID,
...
func (m *postgresDBRepo) InsertBlockForRoom(id int, startDate time.Time) error {
	...
	query := `insert into room_restrictions (start_date, end_date, room_id, restriction_id, created_at, updated_at) value ($1, $2, $3, $4, $5,$ 6)`
